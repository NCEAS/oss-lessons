...
Interative maps (think Google Maps) have become very popular on the web on news sites and all kinds of reporting, including science reporting.
...
It turns out we can actually create these kinds of maps and pages entirely from within R!
...
`leaflet` comes built in with some default ones:

```{r}
points_in_gom$type <- ifelse(runif(npoints) > 0.75, "ship", "pirate")
...
m %>% 
  addLegend("topright", pal = pal, values = ~ALAND,
            title = "Land area",
...
```{r}
library(ggmap)
geocode("Juneau, AK")("Juneau, Alaska")
```
