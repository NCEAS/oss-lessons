...
str_detect(phone_numbers, "\\d\\d\\d[ \\.-]?\\d\\d\\d[ \\.-]?\\d\\d\\d\\d")
```
...
```{r}
words <- c("apple", "crypt", "bike", "banana", "pygmy")
str_detect(words, "[aeiuo]+")
str_extract_all(words, "[aeiuo]+")
```
...
TODO: Have the group figure out you can put * onto [].
TODO: Have the group str_split on fixed chars and a regex
